...
// Główna baza klientów ze zgodą RODO
export const customers = pgTable("customers", {
  ...
  notes: text("notes"), // Notatki o kliencie
...
// Stara tabela contacts - pozostawiam dla kompatybilności
export const contacts = pgTable("contacts", {
  ...
  customerId: integer("customer_id").references(() => customers.id), // Powiązanie z nową bazą
  newsletterConsent: boolean("newsletter_consent").default(false).notNull(),
  marketingConsent: boolean("marketing_consent").default(false).notNull(),
...
// Tabela do śledzenia zgodności RODO
export const gdprConsents = pgTable("gdpr_consents", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").references(() => customers.id).notNull(),
  consentType: text("consent_type", { 
    enum: ["gdpr_basic", "marketing", "newsletter", "sms", "cookies", "analytics"] 
  }).notNull(),
  ...
  legalBasis: text("legal_basis").notNull(), // "consent", "legitimate_interest", "contract", etc.
  description: text("description"), // Opis zgody w języku polskim
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// Tabela żądań RODO (prawo do zapomnienia, eksport danych, etc.)
export const gdprRequests = pgTable("gdpr_requests", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").references(() => customers.id).notNull(),
  requestType: text("request_type", { 
    enum: ["data_export", "data_deletion", "data_rectification", "data_portability", "consent_withdrawal"] 
  }).notNull(),
  status: text("status", { 
  ...
  processedBy: text("processed_by"), // ID administratora
...
export const orders = pgTable("orders", {
  ...
  status: text("status").notNull().default("pending"), // pending, accepted, confirmed, preparing, ready, completed, cancelled, rejected
  ...
  adminNotes: text("admin_notes"), // notes from admin when accepting/rejecting
  adminQuestions: text("admin_questions"), // questions from admin to customer
  customerResponse: text("customer_response"), // customer response to admin questions
...
// API Keys storage table for secure encrypted key management
export const apiKeys = pgTable("api_keys", {
  id: serial("id").primaryKey(),
  keyName: text("key_name").unique().notNull(), // OPENAI_API_KEY, STRIPE_SECRET_KEY, etc.
  encryptedValue: text("encrypted_value").notNull(), // AES-256 encrypted key
  keyType: text("key_type", { enum: ["openai", "stripe_secret", "stripe_public", "sendgrid", "google_analytics"] }).notNull(),
...
export const insertCustomerSchema = createInsertSchema(customers).omit({
  ...
  gdprConsent: z.boolean().refine(val => val === true, "Zgoda RODO jest wymagana"),
