...
import { AlertCircle, TrendingUp, Users, Eye, MessageSquare, Hash, Globe, Search, Award, Zap, Image, Calendar as CalendarIcon, Send, Facebook, Instagram, Youtube, Linkedin, Clock, Camera, Palette, FileImage, Share2, Settings, Gift } from "lucide-react";
...
Zapraszamy do skorzystania z naszego systemu zamówień online z wygodnym odbiorem oraz do zakupu naszych autorskich sosów w sklepie Stefano.`,
  ...
  const [detailedStats, setDetailedStats] = useState({
    ...
    avatars: {
      'cyber-medrzec': { orders: 342, popularity: '28%' },
      'cyber-rycerz': { orders: 298, popularity: '25%' },
      'cyber-czarownik': { orders: 267, popularity: '22%' },
      'cyber-wojowniczka': { orders: 298, popularity: '25%' }
    },
    topDishes: [
  ...
  const loadSocialPosts = () => {
    const posts: SocialPost[] = [
      {
        id: '1',
        content: 'Dzisiaj specjalna promocja na nasze autorskie pizze! Zamów teraz i odbierz ze swoim AWATAREM!',
  ...
  const generateSEOContent = () => {
    ...
    const socialPosts = [
      "🍕 Dzisiaj specjalna promocja na nasze autorskie pizze! Zamów teraz i odbierz ze swoim AWATAREM! #StefanoRestaurant #PizzaBełchatów #DailySpecial",
      ...
      <div className="container mx-auto">
        ...
        <Tabs defaultValue="stats" className="space-y-6">
          ...
          <TabsContent value="detailed-stats" className="space-y-6">
            ...
            <Card className="bg-gray-900 border-gray-700">
              <CardHeader>
                <CardTitle className="text-stefano-gold">Statystyki Avatarów</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {Object.entries(detailedStats.avatars).map(([avatar, data]) => (
                    <div key={avatar} className="bg-gray-800 rounded-lg p-4 text-center">
                      <div className="text-2xl mb-2">
                        {avatar === 'cyber-medrzec' ? '🧙‍♂️' : 
                         avatar === 'cyber-rycerz' ? '⚔️' :
                         avatar === 'cyber-czarownik' ? '🔮' : '🛡️'}
                      </div>
                      <div className="font-semibold capitalize">{avatar.replace('cyber-', '').replace('-', ' ')}</div>
                      <div className="text-lg font-bold text-stefano-gold">{data.orders}</div>
                      <div className="text-sm text-gray-400">{data.popularity} popularności</div>
                    </div>
                  ))}
                </div>
              </CardContent>
                      ...
                      {Object.entries(aboutUsContent.workingHours).map(([day, hours]) => (
                        <div key={day} className="flex gap-2">
                          <Input
                            value={day}
                            onChange={(e) => {
                              const newHours = {...aboutUsContent.workingHours};
                              delete newHours[day];
                              newHours[e.target.value] = hours;
                              setAboutUsContent(prev => ({...prev, workingHours: newHours}));
                            }}
                ...
                <CardContent className="space-y-4">
                  <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                    ...
                    <ul className="text-sm space-y-1">
                      ...
                      <li>📱 80% zamówień przez avatary</li>
                    </ul>
                  </div>
                      ...
                      <div key={post.id} className="bg-gray-800 p-3 rounded-lg">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex-1">
                            <p className="text-sm">{post.content.substring(0, 80)}...</p>
